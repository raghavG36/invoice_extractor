...
- **Pipeline A:** Runs OCR (Tesseract or Azure) on each invoice image, then sends the text to an LLM (OpenAI or local Ollama) to extract structured fields.
- **Pipeline B:** Runs the same OCR, then uses regex and heuristics to extract the same fields without an LLM.
- **Flow:** For each image, both pipelines run → outputs are normalized (dates, numbers) → results are compared field-by-field → a **reconciled** row (preferring Pipeline A with fallbacks) is written to `output.csv`, and the A vs B comparison is written to `comparison_report.csv`. A short accuracy summary is printed at the end.
...
| `INVOICE_IMAGE_END` | End index for image range (default: `349`) |
| `INVOICE_IMAGE_EXT` | Image extension (default: `jpg`) |
| `INVOICE_OCR_BACKEND` | `pytesseract` (default) or `azure` |
...
Place invoice images in the configured directory (default: `invoice_extractor/images/`) with names like `batch1-0348.jpg`, `batch1-0349.jpg`, etc. Adjust `INVOICE_IMAGE_PREFIX`, `INVOICE_IMAGE_START`, `INVOICE_IMAGE_END`, and `INVOICE_IMAGE_EXT` to match your naming and range.
...
```
invoice-extraction/
├── main.py                 # Entry point (python main.py); calls invoice_extractor.main
├── requirements.txt
├── .env.example
├── README.md
└── invoice_extractor/
    ...
    ├── pipeline_a/         # OCR (Tesseract/Azure) + OpenAI LLM extraction
...
1. Install and start [Ollama](https://ollama.com), then pull a model: e.g. `ollama pull llama3.2`
2. In `.env` set: `INVOICE_LLM_PROVIDER=ollama`. Optionally set `OLLAMA_MODEL` (default `llama3.2`) and `OLLAMA_BASE_URL` (default `http://localhost:11434/v1`).
...
1. Add `azure-ai-vision-imageanalysis>=1.0.0b1` to `requirements.txt` and install.
2. Set in `.env`: `INVOICE_OCR_BACKEND=azure`, `AZURE_VISION_KEY`, `AZURE_VISION_ENDPOINT`.

Pipeline B still uses the same OCR layer (Tesseract or Azure, depending on config).

## Google Colab

You can run the pipeline in [Google Colab](https://colab.research.google.com) (no local Ollama; use **Gemini** or OpenAI).

1. Open **`colab_setup.ipynb`** in Colab (upload the repo or open from GitHub).
...
3. Run the install cell: it installs Tesseract and `pip install -r requirements.txt`.
